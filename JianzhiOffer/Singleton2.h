...
//2.getInstance返回指针而不是引用:如果返回的是引用，执行Singleton2* s21 = Singleton2::getInstance()时要执行一次复制即调用Singleton2(const Singleton2&)，这不符合单例模式不可复制的特点而且这个复制构造函数被声明为private，不可访问。
